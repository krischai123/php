...
<body>
    ...
    if($_GET && isset($_GET['rid'])){
        ...
        $rate_insert = 'SELECT * FROM mm_genres WHERE genres_id ='.$rate_id;
        $rate_result = $conn->query($rate_insert);
        $rate = $rate_result->fetch_assoc();

        $rate_name1 = $rate['genres_title'];
    ...
    if ($_POST && isset($_POST['txexdata']) && isset($_POST['edit_id'])) {
        ...
        echo '<br>';
        $rate_update = 'UPDATE mm_genres SET genres_title = "'.$rate_name.'" WHERE genres_id ='.$rate_id;
